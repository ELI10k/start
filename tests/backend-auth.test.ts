...
test("magic links are confirmed inside the app before OTP verification",async()=>{const [confirmation,form,acceptance,onboarding,config]=await Promise.all([readFile(new URL("../app/auth/confirm-link/page.tsx",import.meta.url),"utf8"),readFile(new URL("../components/auth/MagicLinkConfirmation.tsx",import.meta.url),"utf8"),readFile(new URL("../app/auth/accept-link/route.ts",import.meta.url),"utf8"),readFile(new URL("../app/actions/onboarding.ts",import.meta.url),"utf8"),readFile(new URL("../supabase/config.toml",import.meta.url),"utf8")]);assert.match(confirmation,/MagicLinkConfirmation/);assert.match(form,/action="\/auth\/accept-link"/);assert.match(form,/isNativePlatform/);assert.match(form,/requestSubmit/);assert.match(acceptance,/verifyOtp\(\{ token_hash: tokenHash, type: "magiclink" \}\)/);assert.match(acceptance,/activate_current_device/);assert.match(onboarding,/auth\/confirm-link/);assert.match(config,/auth\.email\.template\.magic_link/) });
test("coached clients bypass self-service onboarding while independent clients retain it",async()=>{const [invite,link,callback,onboarding,proxy]=await Promise.all([readFile(new URL("../app/auth/accept-invite/route.ts",import.meta.url),"utf8"),readFile(new URL("../app/auth/accept-link/route.ts",import.meta.url),"utf8"),readFile(new URL("../app/auth/callback/route.ts",import.meta.url),"utf8"),readFile(new URL("../app/onboarding/page.tsx",import.meta.url),"utf8"),readFile(new URL("../proxy.ts",import.meta.url),"utf8")]);for(const source of [invite,link,callback,onboarding,proxy])assert.match(source,/coach_client_relationships/);assert.match(invite,/onboarding_completed:true/);assert.match(invite,/return redirect\("\/"\)/);assert.match(link,/if \(!relationship && !onboarding\?\.onboarding_completed\)/);assert.match(callback,/if \(!relationship && !onboarding\?\.onboarding_completed\)/);assert.match(onboarding,/if\s*\(relationship\)\s*redirect\("\/"\)/);assert.match(proxy,/if \(!relationship && !onboarding\?\.onboarding_completed\)/) });

...
test("official repository contains every imported food and supports barcode search", () => { assert.equal(foods.filter((food) => /^\d+$/.test(food.id)).length, 339); assert.equal(filterAndSortFoods(foods, "7290014758681")[0]?.id, "1"); });

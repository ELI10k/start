...
test("food search covers name, brand and category and preserves source", () => { const all = foodRepository.getAll(); const food = all[0]; for (const term of [food.name, food.brand!, food.category]) assert.ok(queryFoods(all, { search: term }).some((item) => item.id === food.id)); assert.equal(all.filter((item) => /^\d+$/.test(item.id)).length, 358); });

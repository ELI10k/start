...
import { saveContentCategory, setContentItemStatus } from "@/app/actions/content";
...
export default async function CoachContentPage() {
  ...
  const [items, categories, progressResult] = await Promise.all([listCoachContent(), listContentCategories(true),supabase.from("content_progress").select("content_item_id,client_id,progress_percent,last_viewed_at")]);
  ...
  const courses = categories.flatMap((category) => {
    ...
    const activity=progress.filter((row)=>lessonIds.has(row.content_item_id));
    return lessons.length ? [{ ...category, lessons, cover: lessons.find((item) => item.thumbnailUrl)?.thumbnailUrl ?? null, viewers:new Set(activity.map((row)=>row.client_id)).size, completed:activity.filter((row)=>Number(row.progress_percent)===100).length }] : [];
  });
        ...
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          {courses.map((course, courseIndex) => <article key={course.id} className="group overflow-hidden rounded-[28px] border border-[#E5E7E5] bg-[#FFFFFF]">
            <div className="relative aspect-video overflow-hidden bg-[#ECFDF3]">
              ...
              <div className="absolute inset-x-0 bottom-0 p-5"><span className="rounded-full bg-[#16A34A] px-3 py-1 text-xs font-black text-[#FFFFFF]">קורס</span><h2 className="mt-3 text-2xl font-black text-[#FFFFFF]">{course.name}</h2><p className="mt-1 text-sm text-[#FFFFFF]/80">{course.lessons.length} שיעורים · {course.viewers} צופים · {course.completed} השלמות</p></div>
            </div>
            <div className="p-5">
              ...
              <details className="mt-4 rounded-2xl border border-[#E5E7E5] bg-[#F7F8F7]">
                ...
                <div className="space-y-2 border-t border-[#E5E7E5] p-3">
                  <form action={saveContentCategory} className="grid gap-2 rounded-xl border border-[#E5E7E5] bg-white p-3 sm:grid-cols-2">
                    <input type="hidden" name="id" value={course.id}/><label className="text-xs font-bold">שם הקורס<input name="name" defaultValue={course.name} className="nutrition-input mt-1"/></label><label className="text-xs font-bold">סדר<input name="sortOrder" type="number" min="0" defaultValue={course.sortOrder} className="nutrition-input mt-1"/></label><label className="text-xs font-bold sm:col-span-2">תיאור<input name="description" defaultValue={course.description??""} className="nutrition-input mt-1"/></label><label className="flex items-center gap-2 text-xs font-bold"><input name="active" type="checkbox" defaultChecked={course.active}/>קורס פעיל</label><SubmitButton idle="שמירת פרטי הקורס" pending="שומרים…" className="premium-secondary-button"/>
                  </form>

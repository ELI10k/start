...
export default async function CoachClientsPage({
  ...
  if (params.view === "archived") {
    ...
    return (
      <main className="client-app-content">
        ...
        <Link href="/coach/clients" className="chip">
          חזרה ללקוחות הפעילים
        </Link>

        {archived.length ? (
          <div className="mt-4 grid gap-3">
            {archived.map((client) => (
              <article key={client.id} className="premium-card">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0">
                    <h2 className="truncate text-lg font-black">
                      {client.full_name}
                    </h2>
                    <p className="mt-1 truncate text-xs text-[#5B5F5B]">
                      {client.email}
                ...
                <dl className="compact-data-list mt-3">
                  <div>
                    <span>תחילת ליווי</span>
                    <strong>
                      {client.startDate ? date(client.startDate) : "אין נתון"}
                    </strong>
                  </div>
                  <div>
                    <span>הועבר לארכיון</span>
                    <strong>
                      {client.archivedAt ? date(client.archivedAt) : "אין נתון"}
                    </strong>
                  </div>
                </dl>
              ...
              readable until they are restored. Saying so beats a link that 404s. */}
                <p className="mt-3 text-xs text-[#5B5F5B]">
                  תיק הלקוח ייפתח שוב לאחר שחזור. הנתונים שמורים במלואם.
                </p>
              </article>
            ))}
          </div>
  ...
  const sort =
    params.sort === "checkin" || params.sort === "weight"
      ? params.sort
      : "name";
  ...
  const status = [
    ...
    ? params.status
    : "all";
  const result = await listCoachDashboardClients(auth.id, {
    query: params.q,
    sort,
    status,
    page,
  ...
  const pageHref = (target: number) =>
    `/coach/clients?${new URLSearchParams({ ...(params.q ? { q: params.q } : {}), sort, ...(status !== "all" ? { status } : {}), page: String(target) })}`;
  const sortHref = (target: string) =>
    `/coach/clients?${new URLSearchParams({ ...(params.q ? { q: params.q } : {}), sort: target })}`;
    ...
    <main className="client-app-content">
      ...
      <div className="chip-row mt-3">
        {[
          ...
          { value: "attention", label: "דורש תשומת לב" },
          { value: "active", label: "פעילים" },
          { value: "inactive", label: "לא פעילים" },
          { value: "no-menu", label: "ללא תפריט" },
          { value: "no-workout", label: "ללא אימונים" },
        ].map((item) => (
          <Link
            key={item.value}
            href={`/coach/clients?${new URLSearchParams({ ...(params.q ? { q: params.q } : {}), sort, ...(item.value !== "all" ? { status: item.value } : {}) })}`}
            className="chip"
        ...
        <Link href="/coach/clients?view=archived" className="chip">
          לקוחות בארכיון
        </Link>
      </div>
      ...
      {result.items.length ? (
          ...
          {result.items.map((client) => (
            <Link href={`/coach/clients/${client.id}`} key={client.id}>
              ...
              <span className="app-list__main">
                ...
                <span>
                  {client.latestWeight
                    ? `${client.latestWeight} ק״ג`
                    : "אין מדידה"}{" "}
                  · צ׳ק־אין {date(client.lastCheckInAt)}
                </span>
                <span className="mt-1 flex flex-wrap gap-1">
                  {!client.hasMenu && (
                    <em className="pill not-italic">ללא תפריט</em>
                  )}
                  {!client.hasWorkout && (
                    <em className="pill not-italic">ללא אימונים</em>
                  )}
                  {client.lastLoginAt && (
                    <em className="text-xs not-italic text-[#5B5F5B]">
                      פעילות {date(client.lastLoginAt)}
                    </em>
                  )}
                </span>
              </span>
              <span className={pills[client.dashboardStatus]}>
              ...
              <span className="hidden shrink-0 text-sm font-bold text-[#16A34A] sm:inline">
                פתיחת תיק
              </span>
        ...
        <StateBlock
          ...
          action={
            <Link href="/coach/clients/new" className="premium-primary-button">
              לקוח חדש
            </Link>
          }
        />
      ...
      {result.total > result.pageSize && (
        <nav
          ...
          aria-label="עמודי לקוחות"
        >
          {result.page > 1 && (
            <Link className="chip" href={pageHref(result.page - 1)}>
              הקודם
            </Link>
          )}
          <span className="text-sm text-[#5B5F5B]">
            עמוד {result.page} מתוך {Math.ceil(result.total / result.pageSize)}
          </span>

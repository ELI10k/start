-- Rollback for 202608190004_substituted_exercise.sql.
--
-- Restores both functions to their 202608180002 definitions and drops the column.
-- The recorded substitutions go with it: without the column there is nowhere to
-- say "the leg press was done on a different machine", and exercise_id already
-- holds the prescribed exercise, which is the reading the older schema had.

begin;

alter table public.workout_session_exercises drop column if exists performed_exercise_id;

...
-- Plans saved before food groups existed were preserved in one legacy group.
-- Split those alternatives by their authoritative calculated macro values.
insert into public.meal_food_groups(meal_id,group_type,sort_order)
select distinct i.meal_id,
  case
    when i.calculated_fat*9>i.calculated_protein*4 and i.calculated_fat*9>i.calculated_carbohydrates*4 then 'fat'
    when i.calculated_carbohydrates*4>i.calculated_protein*4 then 'carbohydrate'
    else 'protein'
  end,
  case
    when i.calculated_fat*9>i.calculated_protein*4 and i.calculated_fat*9>i.calculated_carbohydrates*4 then 2
    when i.calculated_carbohydrates*4>i.calculated_protein*4 then 1
    else 0
  end

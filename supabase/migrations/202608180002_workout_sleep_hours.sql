-- How many hours the client slept the night before the workout.
--
-- Sleep is the first thing a coach asks about when the numbers drop, and the only
-- place it was recorded was the weekly check-in - a 1-5 rating, a week after the
-- session it would have explained. This records the actual hours next to the
-- workout they belong to. Both session writers carry the whole session as jsonb,
-- so both learn the new field the same way; every guard in them is unchanged.

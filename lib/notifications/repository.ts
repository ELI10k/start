...
const RETIRED_PROTEIN_FOCUS_TITLE = "הפוקוס שלך עכשיו: חלבון";
...
export async function getNotificationCenter() {
  ...
  const [{ data: notifications, error: notificationError }, { data: preferences, error: preferenceError }] = await Promise.all([
    supabase.from("notifications").select("id,actor_id,category,type,title,body,href,created_at,read_at").eq("recipient_id", user.id).neq("title", RETIRED_PROTEIN_FOCUS_TITLE).order("created_at", { ascending: false }).limit(80),
...
export async function getUnreadNotificationCount(excludeTypes: readonly string[] = []) {
  ...
  let query = supabase
    ...
    .neq("title", RETIRED_PROTEIN_FOCUS_TITLE)
    .is("read_at", null);

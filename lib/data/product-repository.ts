...
export async function getActiveClientMenu(
  ...
  const { data: allMeals, error: mealError } = await supabase
    .from("meals")
    .select("id,title,sort_order,day_index")
  ...
  const [{ data: items, error: itemError }, { data: log, error: logError }] =
    await Promise.all([
      mealIds.length
        ? supabase
            .from("meal_items")
            .select(
              "id,meal_id,food_id,amount,calculated_calories,calculated_protein,calculated_carbohydrates,calculated_fat,foods(name)",
            )
            .in("meal_id", mealIds)
            .order("sort_order")
  ...
  return {
    ...
    meals: meals.map((meal) => {
      ...
      return {
        ...
        completed:
          mealItems.length > 0 && mealItems.every((item) => item.eaten),
        items: mealItems,
      };
    }),
  };
...
export async function listDatabaseFoods() {
  ...
  const { data, error } = await supabase
    .from("foods")
    .select("id,name,brand,category,calories,protein,carbs,fat,serving_label")
    .order("name");
  if (error) throw error;
  return data ?? [];
}

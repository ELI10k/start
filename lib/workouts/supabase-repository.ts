...
export type AssignmentInput = Readonly<{clientId:string;programId:string;startDate:string;endDate?:string;weeklyFrequency:number;coachNote?:string;replaceActive?:boolean}>;
...
export function createSupabaseWorkoutRepository(){
  ...
  const load=async():Promise<WorkoutLoadResult>=>{
    ...
    const exerciseResults=(sessionId:string):ActiveExerciseResult[]=>resultRows.filter((row)=>row.session_id===sessionId).map((row)=>({workoutExerciseId:text(row.workout_exercise_id),exerciseId:text(row.exercise_id),performedExerciseId:optionalText(row.performed_exercise_id),skipped:Boolean(row.skipped),completed:Boolean(row.completed),difficulty:row.difficulty==="easy"||row.difficulty==="medium"||row.difficulty==="hard"?row.difficulty:undefined,warmupCompletedPercents:Array.isArray(row.warmup_completed_percents)?row.warmup_completed_percents.map(Number):[],sets:setRows.filter((set)=>set.session_id===sessionId&&set.workout_exercise_id===row.workout_exercise_id).map(mapSet)}));
    const activeSessions:ActiveWorkoutSession[]=sessionRows.filter((row)=>row.status==="active").map((row)=>({id:text(row.id),clientId:text(row.client_id),assignmentId:text(row.assignment_id),programId:text(row.program_id),dayId:text(row.day_id),startedAt:text(row.started_at),currentExerciseIndex:Number(row.current_exercise_index),restEndsAt:optionalText(row.rest_ends_at),workoutNote:optionalText(row.workout_note),perceivedDifficulty:numberValue(row.perceived_difficulty) as ActiveWorkoutSession["perceivedDifficulty"],energy:numberValue(row.energy) as ActiveWorkoutSession["energy"],sleepHours:numberValue(row.sleep_hours),exerciseResults:exerciseResults(text(row.id))}));
    const completedWorkouts:CompletedWorkout[]=sessionRows.filter((row)=>row.status==="completed").map((row)=>({id:text(row.completion_id),clientId:text(row.client_id),assignmentId:text(row.assignment_id),programId:text(row.program_id),dayId:text(row.day_id),startedAt:text(row.started_at),completedAt:text(row.completed_at),durationSeconds:Number(row.duration_seconds),exerciseResults:exerciseResults(text(row.id)),workoutNote:optionalText(row.workout_note),perceivedDifficulty:numberValue(row.perceived_difficulty) as CompletedWorkout["perceivedDifficulty"],energy:numberValue(row.energy) as CompletedWorkout["energy"],sleepHours:numberValue(row.sleep_hours),totalVolume:Number(row.total_volume)}));
  ...
  return{
    load,
    assign:async(input:AssignmentInput)=>rpc("assign_workout_program",{p_program_id:input.programId,p_client_id:input.clientId,p_start_date:input.startDate,p_end_date:input.endDate??null,p_weekly_frequency:input.weeklyFrequency,p_coach_note:input.coachNote??"",p_replace_active:input.replaceActive!==false}),
    ...
    // Same payload, a session that is already closed: filling in the numbers afterwards.
    updateCompletedSession:async(workout:CompletedWorkout)=>rpc("update_completed_workout",{p_workout:workout}),

...
export default function WorkoutSession({programId,dayId}:{programId:string;dayId:string}){
  ...
  const begin=async()=>{if(isStarting)return;if(!assignment){setWarning("לא נמצאה הקצאת תוכנית פעילה.");return}if(anySession&&!session){setWarning("כבר קיים אימון פעיל אחר.");return}setIsStarting(true);try{if(await startSession(makeSession()))track("workout_started",{exercises:ordered.length});else setWarning("לא ניתן להתחיל שני אימונים במקביל.")}finally{setIsStarting(false)}};

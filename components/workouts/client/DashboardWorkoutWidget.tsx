...
export default function DashboardWorkoutWidget() {
  ...
  const today = program ? getTodayWorkoutDay(program, snapshot.completedWorkouts, currentClientId, israelDateKey(), snapshot.scheduleChanges.filter((c) => c.clientId === currentClientId && c.status === "skipped").map((c) => ({ dayId: c.dayId, date: c.originalDate }))) : undefined;

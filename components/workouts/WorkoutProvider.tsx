...
import type { ActiveWorkoutSession, AssignmentStatus, CoachWorkoutNote, CompletedWorkout, Exercise, WorkoutPreferences, WorkoutProgram, WorkoutRepositorySnapshot } from "@/lib/workouts/types";
...
export function WorkoutProvider({children}:{children:React.ReactNode}){
  ...
  const refresh=useCallback(async()=>{const loaded=await repository.load();setSnapshot(loaded.snapshot);setCurrentClientId(loaded.role==="client"?loaded.currentUserId:"");setPersistenceError("");return loaded},[repository]);
  useEffect(()=>{let active=true;repository.load().then((loaded)=>{if(active){setSnapshot(loaded.snapshot);setCurrentClientId(loaded.role==="client"?loaded.currentUserId:"");setPersistenceError("")}}).catch(()=>{if(active){setSnapshot(emptyWorkoutSnapshot);setCurrentClientId("");setPersistenceError("יש להתחבר כדי לטעון את נתוני האימונים.")}}).finally(()=>{if(active){setLoadedAuthScope(authScope);setLoading(false)}});return()=>{active=false}},[authScope,repository]);
  const fail=useCallback(()=>{setPersistenceError("השמירה ב-Supabase נכשלה. יש לרענן ולנסות שוב.");return false},[]);
  const value=useMemo<ContextValue>(()=>({
    snapshot,currentClientId,loading:loading||loadedAuthScope!==authScope,persistenceError,
  ...
  }),[authScope,currentClientId,fail,loadedAuthScope,loading,persistenceError,refresh,repository,snapshot]);

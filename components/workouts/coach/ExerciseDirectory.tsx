...
import { queryExercises } from "@/lib/workouts/repository";
    ...
    {results.length ? <div className="mt-3 grid gap-3 md:grid-cols-2">{results.map((exercise) => {
      ...
      return <article key={exercise.id} className="rounded-[22px] border border-[#E5E7E5] bg-[#FFFFFF] p-5">
        <div className="flex justify-between gap-3">
          <Link href={detailHref} className="flex min-w-0 items-center gap-3 rounded-xl focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#16A34A]" aria-label={`פתיחת פרטי ${exercise.name}`}>
            <ExerciseThumbnail exercise={exercise}/><div><h2 className="text-lg font-black">{exercise.name}</h2><p className="mt-1 text-xs text-[#5B5F5B]">{[exercise.primaryMuscleGroup, exercise.equipment, exercise.category, exercise.difficulty].filter(Boolean).join(" · ")}</p></div>
          </Link>
          <span className="text-xs text-[#16A34A]">{exercise.status === "active" ? "פעיל" : "בארכיון"}</span>
        </div>
        <p className="mt-3 line-clamp-2 text-sm text-[#5B5F5B]">{exercise.executionNotes ?? "לא סופקו הערות ביצוע"}</p>
        <div className="mt-3 flex flex-wrap items-center gap-4"><Link href={detailHref} className="inline-flex min-h-11 items-center text-sm text-[#16A34A]">פרטים וסרטון הסבר טכניקה</Link><ExerciseGuidanceButton exercise={exercise} variant="link"/>{!exercise.cues.length && !exercise.commonMistakes.length && <span className="pill">ללא דגשים</span>}</div>
        <p className="text-[10px] text-[#3F433F]">מקור: {exercise.sourceWorkbooks.join(", ")}</p>
      </article>;
    })}</div> : <div className="mt-4 rounded-[22px] border border-dashed border-[#E5E7E5] p-10 text-center text-[#5B5F5B]">{all.length ? "לא נמצאו תרגילים התואמים לסינון." : "קובצי בנק התרגילים המאושרים אינם נמצאים במאגר ולכן לא הומצאו תרגילים חלופיים."}</div>}
  </>;
}

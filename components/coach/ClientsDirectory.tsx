...
export default function ClientsDirectory({ clients }: { clients: Client[] }) {
  ...
  return <>
    ...
    <div className="mb-7 flex items-center gap-2 overflow-x-auto pb-2 [scrollbar-width:none]">
      ...
      {filters.map((item) => <button key={item.value} onClick={() => setFilter(item.value)} aria-pressed={filter === item.value} className={`shrink-0 rounded-full border px-4 py-2 text-xs font-bold transition ${filter === item.value ? "border-[#16A34A] bg-[#16A34A] text-[#FFFFFF]" : "border-[#303030] bg-[#FFFFFF] text-[#5B5F5B] hover:border-zinc-600 hover:text-[#0B0B0B]"}`}>{item.label}</button>)}
    </div>

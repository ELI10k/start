...
export default function DailyCoachCard({ message }: { message: DailyCoachMessage }) {
  return <section className="premium-card mb-6" aria-labelledby="daily-coach-title">
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div><p className="text-xs font-black tracking-[.18em] text-[#16A34A]">START DAILY COACH</p><h2 id="daily-coach-title" className="mt-2 text-xl font-black">{message.title}</h2><p className="mt-2 text-sm leading-6 text-[#5B5F5B]">{message.summary}</p></div>

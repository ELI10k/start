...
function TrendChart({ title, unit, points }: { title: string; unit: string; points: readonly Point[] }) {
  ...
  return (
    <section className="rounded-[24px] border border-[#292929] bg-[#151515] p-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="font-black">{title}</h2>
          <p className="mt-1 text-sm text-zinc-500">{latest} {unit} · שינוי {change > 0 ? "+" : ""}{change} {unit}</p>
        </div>
        <span className="text-xs text-zinc-600">{points.length} מדידות</span>
      </div>
      <svg className="mt-5 h-36 w-full overflow-visible" viewBox="0 0 100 100" role="img" aria-label={title} preserveAspectRatio="none">
        ...
        <polyline fill="none" points={linePoints(points)} stroke="#D4AF37" strokeWidth="2.5" vectorEffect="non-scaling-stroke" />
      </svg>
      <div className="mt-2 flex justify-between text-xs text-zinc-600"><span>{points[0].date}</span><span>{points.at(-1)?.date}</span></div>
    </section>
  );
}
...
export default function PersistedProgressHistory({ entries }: { entries: readonly ProgressEntry[] }) {
  ...
  if (!ordered.length) {
    return <p className="rounded-[24px] border border-dashed border-[#333] p-10 text-center text-zinc-500">עדיין אין מדידות.</p>;
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="overflow-x-auto rounded-[24px] border border-[#292929] bg-[#151515]">
        <table className="w-full min-w-[440px] text-right text-sm">
          <thead className="text-zinc-500"><tr><th className="p-4">תאריך</th><th>משקל</th><th>היקף טבור</th><th className="p-4">הערה</th></tr></thead>
          <tbody>{[...ordered].reverse().map((entry) => <tr key={entry.id} className="border-t border-white/5"><td className="p-4">{entry.date}</td><td>{entry.weight} ק״ג</td><td>{entry.navel_circumference ?? "—"}</td><td className="p-4 text-zinc-400">{entry.notes ?? "—"}</td></tr>)}</tbody>
        </table>
      </section>
    </div>
  );
